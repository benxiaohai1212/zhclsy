...
nph.Gallery = function (options) { 
	...
	t.$photoPrevLoading.addEvent('load', function(){
		var that = t.$photo;
		
		isPhotoImageLoaded = true;		
		t.$photoLoading.addCss('hidden');
		that.src = this.src;
		'string' !== typeof that.style.maxWidth && t.resize(that, { width: pWidth, height: pHeight });
...
nph.Gallery.prototype = {
	showPhoto: function(p){
		...
		if(info.img != t.$photo.src){
			isPhotoImageLoaded = false;
			setTimeout(function(){
				if(!isPhotoImageLoaded){
					t.$photo.src = "http://img1.cache.netease.com/cnews/img/gallery11/bg06.png";
					t.$photoLoading.removeCss('hidden');
				}
			}, 200);
			t.$photoIndex.attr('innerHTML', index + 1);
			t.$photoPrevLoading.src = info.aimg ? info.aimg : info.img;
